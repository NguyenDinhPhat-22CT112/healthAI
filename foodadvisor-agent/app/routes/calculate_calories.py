...
async def calculate_calories(
    ...
    for food_input in request.foods:
        ...
        food = db.query(Food).filter(
            Food.name.ilike(f"%{food_input.name}%")
        ).first()
        ...
        if food:
            ...
            calo = (food.calo * actual_weight) / 100 if food.calo else 0
            protid = (food.protid * actual_weight) / 100 if food.protid else 0
            lipid = (food.lipid * actual_weight) / 100 if food.lipid else 0
            glucid = (food.glucid * actual_weight) / 100 if food.glucid else 0
